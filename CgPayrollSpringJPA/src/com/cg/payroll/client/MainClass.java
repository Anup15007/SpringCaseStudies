		...
		ApplicationContext context=new ClassPathXmlApplicationContext("payrollBeans.xml");
			...
			while(ch=='y'||ch=='Y') {
				...
				case 2: 
					...
					int id = sc.nextInt();
					Associate associateDetails = payrollServices.getAssociateDetails(id);
					System.out.println(associateDetails);
					break;
				case 3:
					List<Associate> allAssociateDetails = payrollServices.getAllAssociatesDetails();		
					System.out.println(allAssociateDetails);
					break;
				case 4:
					System.out.println("Enter associate id to calculate net salary: ");
					id = sc.nextInt();
					int netSalary = payrollServices.calculateNetSalary(id);
